...
export default function UploadsPage() {
  ...
  const fetchGalleryItems = async () => {
    try {
      const res = await fetch('/admin/api/gallery-items');
  ...
  const handleSaveItem = async (
    ...
    try {
      ...
      const url = id ? `/admin/api/gallery-items?id=${id}` : '/admin/api/gallery-items';
  ...
  const handleBulkDeleteItems = async (ids: string[]) => {
    try {
      const res = await fetch(`/admin/api/gallery-items?ids=${ids.join(',')}`, {
        method: 'DELETE',
      });
